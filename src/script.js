 const i18n = {
    ...
    'zh-TW': {
        ...
        revealFile: "顯示路徑",
        downloadFile: "下載影片",
        filePathAlertTitle: "目前影片檔案路徑",
        copiedToClipboard: "已複製到剪貼簿",
        noFilePath: "無法取得目前攝影機的檔案路徑",
        selectDate: "選擇日期",
        minutes: "分鐘",
        preview: "預覽圖",
        noSignal: "無訊號",
        ...
        toggleDay: "切換至日間模式",
        toggleNight: "切換至夜間模式",
        invalidFolder: "這似乎不是有效的 TeslaCam 目錄。請確認您選擇的是包含 RecentClips、SavedClips 等子資料夾的 TeslaCam 目錄。",
        clipVideo: "剪輯影片",
        confirmClip: "確認剪輯範圍",
        exportClip: "匯出影片片段",
        clipDuration: "選取時長:",
        ...
        blinkerOff: "關",
        autopilotNone: "無",
        autopilotSelfDriving: "全自動輔助駕駛 (FSD)",
        autopilotAutosteer: "自動輔助轉向",
        autopilotTACC: "主動式定速巡航",
        moreOptions: "更多選項",
        metadataDetail: "詳細數據",
        vehicleStatus: "車輛狀態",
        drivingAssist: "駕駛輔助",
...
class MetadataOverlayGenerator {
    ...
    async generateOverlayPngs(allMetadata, clipSegments, workDir, width = 1920, height = 1080, progressCallback = null, language = 'zh-TW') {
        ...
        progressCallback?.(language === 'zh-TW' ? `生成 ${uniqueStates.size} 個中繼資料覆蓋層...` : `Generating ${uniqueStates.size} metadata overlays...`);
        ...
        for (const [key, data] of uniqueStates) {
            try {
                ...
                if (idx % 20 === 0) {
                    progressCallback?.(language === 'zh-TW' ? `生成覆蓋層 ${idx}/${uniqueStates.size}...` : `Generating overlay ${idx}/${uniqueStates.size}...`);
...
class MetadataManager {
    ...
    updateStatsDisplay(d) {
        ...
        s.brake.textContent = d.brakeApplied 
            ? (lang === 'zh-TW' ? '踩下' : 'Applied') 
            : (lang === 'zh-TW' ? '未踩' : 'Released');
        ...
        if (blinkerLeft && blinkerRight) {
            s.blinker.textContent = lang === 'zh-TW' ? '雙閃' : 'Hazard';
            s.blinker.className = 'stats-value highlight-yellow';
        } else if (blinkerLeft) {
            s.blinker.textContent = lang === 'zh-TW' ? '← 左轉' : '← Left';
            s.blinker.className = 'stats-value highlight-green';
        } else if (blinkerRight) {
            s.blinker.textContent = lang === 'zh-TW' ? '右轉 →' : 'Right →';
            s.blinker.className = 'stats-value highlight-green';
        } else {
            s.blinker.textContent = lang === 'zh-TW' ? '關' : 'Off';
            s.blinker.className = 'stats-value';
        }
        
        // Autopilot
        const apMap = {
            'NONE': lang === 'zh-TW' ? '無' : 'None',
            'SELF_DRIVING': 'FSD',
            'AUTOSTEER': lang === 'zh-TW' ? '自動轉向' : 'Autosteer',
            'TACC': 'TACC'
        };
...
class ModernVideoControls {
    ...
    updateRealTimeClock() {
        ...
        try {
            ...
            this.realTimeClock.textContent = '錯誤';
...
class VideoClipProcessor {
    ...
    async loadFFmpeg(progressCallback) {
        ...
        if (!FFmpegLib) {
            progressCallback?.('載入 FFmpeg 程式庫...');
        ...
        if (useMultiThread) {
            progressCallback?.('載入 FFmpeg 多執行緒核心模組...');
            console.log('[FFmpeg] Loading FFmpeg WASM core (multi-threaded)...');
        } else {
            progressCallback?.('載入 FFmpeg 核心模組...');
        ...
        try {
            if (useMultiThread) {
                ...
                progressCallback?.('載入 FFmpeg 核心檔案 (使用本機快取)...');
            ...
            if (useMultiThread) {
                ...
                progressCallback?.('多執行緒載入失敗，使用單執行緒模式...');
                const [coreURL, wasmURL] = await Promise.all([
                    this.toBlobURLWithCache(`${baseURL}/ffmpeg-core.js`, 'text/javascript', progressCallback),
                    this.toBlobURLWithCache(`${baseURL}/ffmpeg-core.wasm`, 'application/wasm', progressCallback),
                ]);
    ...
    async fixWebmWithFFmpeg(fileHandle, progressCallback) {
        try {
            progressCallback?.(this.currentLanguage === 'zh-TW' ? '載入 FFmpeg 修復模組...' : 'Loading FFmpeg repair module...');
            const ffmpeg = await this.loadFFmpeg(progressCallback);
            
            // Read the file content
            progressCallback?.(this.currentLanguage === 'zh-TW' ? '讀取影片檔案...' : 'Reading video file...');
            ...
            progressCallback?.(this.currentLanguage === 'zh-TW' ? '修復影片中繼資料...' : 'Repairing video metadata...');
            await ffmpeg.exec([
                '-i', 'input.webm',
                '-c', 'copy',
                '-y',
            ...
            progressCallback?.(this.currentLanguage === 'zh-TW' ? '儲存修復後的影片...' : 'Saving repaired video...');
    ...
    async processWithFFmpegWasm(clipSegments, cameras, startTime, endTime, addTimestamp, mergeGrid, eventStartTime, progressCallback, fileHandle = null, addLogo = false, addAudio = false) {
        ...
        try {
            ...
            // Load Font if needed (once) — use bundled DejaVu Sans Bold
            let fontFile = null;
            if (addTimestamp) {
                progressCallback?.('載入字體...');
                try {
                     let fontData;
                     if (window.__TAURI__) {
            ...
            for (let i = 0; i < clipSegments.length; i++) {
                ...
                progressCallback?.(`處理片段 ${i + 1}/${clipSegments.length}...`);
            ...
            throw new Error('瀏覽器匯出失敗: ' + e.message + "\\n建议使用 Chrome 浏览器或尝试本地应用模式。");
        }
    }
    ...
    async executeFFmpegWithProgress(args, totalDuration, progressCallback, progressPrefix) {
        if (!progressPrefix) {
            progressPrefix = this.currentLanguage === 'zh-TW' ? '編碼中...' : 'Encoding...';
        }
        const tauri = window.__TAURI__;
        ...
        return new Promise((resolve, reject) => {
            ...
            const onFinished = data => {
                ...
                if (code === 0) {
                    ...
                    const lastError = stderr.split('\n').filter(l => l.includes('Error') || l.includes('error')).pop() || '未知錯誤';
                    reject(new Error(`FFmpeg 錯誤 (code ${code}): ${lastError}`));
                }
            };
    ...
    async processWithFFmpeg(clipSegments, camera, progressCallback) {
        ...
        try {
            ...
            progressCallback?.(this.currentLanguage === 'zh-TW' ? `FFmpeg 極速匯出中...` : `FFmpeg Fast Exporting...`);
            
            const totalDuration = clipSegments.reduce((sum, seg) => {
                const dur = (seg.clipEnd || 60) - (seg.clipStart || 0);
                return sum + (dur > 0 ? dur : 0);
            }, 0);
            const output = await this.executeFFmpegWithProgress(args, totalDuration, progressCallback, this.currentLanguage === 'zh-TW' ? '極速匯出...' : 'Fast Exporting...');
    ...
    async processClip(segments, cameras, startTime, endTime, addTimestamp, addMetadata, mergeGrid, eventStartTime, progressCallback, useLocalFFmpeg = false, language = 'zh-TW', fileHandle = null, metadataManager = null, addLogo = false, addAudio = false, colorAdjust = {}) {
        try {
            ...
            if (useLocalFFmpeg && window.__TAURI__) {
                ...
                if (!hasFFmpeg) {
                    throw new Error('未偵測到 FFmpeg，請先安裝 FFmpeg 並確保其在系統 PATH 中');
                }
            ...
            if (fileHandle) {
                 progressCallback?.('正在使用流式匯出模式 (Canvas)...');
            ...
            for (const camera of cameras) {
                progressCallback?.(`處理 ${camera} 攝影機...`);
    ...
    async processWithFFmpegFull(clipSegments, camera, addTimestamp, addMetadata, eventStartTime, progressCallback, addLogo = false, addAudio = false, colorAdjust = {}) {
        ...
        const firstFile = clipSegments[0].segment.files[camera];
        if (!firstFile || !firstFile.path) throw new Error(`${camera} 攝影機檔案路徑未找到`);
        ...
        try {
            ...
            if (addMetadata && this.metadataManager) {
                progressCallback?.(`載入 ${camera} 中繼資料...`);
            ...
            progressCallback?.(this.currentLanguage === 'zh-TW' ? `FFmpeg 處理 ${camera}...` : `FFmpeg Processing ${camera}...`);
            
            // totalDuration already calculated at line 4166
            const output = await this.executeFFmpegWithProgress(args, totalDuration, progressCallback, this.currentLanguage === 'zh-TW' ? `處理 ${camera}...` : `Processing ${camera}...`);
            progressCallback?.(this.currentLanguage === 'zh-TW' ? `處理 ${camera}: 100%` : `Processing ${camera}: 100%`);
            
            console.log('[FFmpeg] Finished processing camera:', camera);
            
            if (output.code !== 0) {
                throw new Error(`FFmpeg 錯誤 (code ${output.code}): ${output.stderr || output.stdout || '未知錯誤'}`);
            }
    ...
    async processWithFFmpegGrid(clipSegments, cameras, addTimestamp, addMetadata, eventStartTime, progressCallback, addLogo = false, addAudio = false, colorAdjust = {}) {
        ...
        if (activeCameras.length === 0) throw new Error('沒有可匯出的影片串流');
        ...
        if (!firstFile) throw new Error('檔案路徑未找到');
        ...
        try {
            ...
            if (addMetadata && this.metadataManager) {
                ...
                if (allMetadata && allMetadata.length > 0) {
                    progressCallback?.(this.currentLanguage === 'zh-TW' ? `生成中繼資料覆蓋層...` : `Generating metadata overlays...`);
                    try {
                        overlayInfo = await metadataOverlayGenerator.generateOverlayPngs(
                            allMetadata,
                            clipSegments,
            ...
            progressCallback?.(this.currentLanguage === 'zh-TW' ? 'FFmpeg 合成四宮格...' : 'FFmpeg Merging Grid...');
            
            const output = await this.executeFFmpegWithProgress(args, totalDuration, progressCallback, this.currentLanguage === 'zh-TW' ? '合成四宮格...' : 'Merging Grid...');
            progressCallback?.(this.currentLanguage === 'zh-TW' ? '合成四宮格: 100%' : 'Merging Grid: 100%');
    ...
    async processVideoWithTimestamp(clipSegments, camera, totalStartTime, totalEndTime, addTimestamp, addMetadata, eventStartTime, progressCallback, fileHandle = null, addLogo = false, addAudio = false, colorAdjust = {}) {
        ...
        progressCallback?.(this.currentLanguage === 'zh-TW' ? `處理 ${camera} 攝影機 (${clipSegments.length} 個片段)...` : `Processing ${camera} camera (${clipSegments.length} segments)...`);
        
        // Load metadata for all segments if addMetadata is enabled
        let allMetadata = [];
        if (addMetadata && this.metadataManager) {
            progressCallback?.(this.currentLanguage === 'zh-TW' ? '載入行車數據...' : 'Loading driving data...');
            allMetadata = await this.loadMetadataForSegments(clipSegments, camera, progressCallback);
        ...
        let bgAudio = null;
        if (addAudio) {
            progressCallback?.(this.currentLanguage === 'zh-TW' ? '載入背景音樂...' : 'Loading background audio...');
        ...
        if (bgAudio) {
            try {
                ...
                source.connect(dest);
                // Do NOT connect to destination — avoid playback during export
        ...
        for (let i = 0; i < clipSegments.length; i++) {
            ...
            if (!videoFile) {
                throw new Error(`${camera} 攝影機在片段 ${i + 1} 中沒有可用的影片檔案`);
            }
            
            progressCallback?.(`載入片段 ${i + 1}/${clipSegments.length}...`);
                    ...
                    const captureFrame = (now, metadata) => {
                        ...
                        if (metadata.mediaTime !== lastFrameTime) {
                            ...
                            if (processedFrames % 30 === 0) {
                                ...
                                progressCallback?.(`處理 ${camera}: ${progress}%`);
                            }
                        }
            ...
            if (fileHandle) {
                try {
                    ...
                    if (!fixed) {
                        ...
                        progressCallback?.('影片已儲存（中繼資料修復跳過）');
                    }
                } catch (e) {
    ...
    async createGridVideoFromSegments(clipSegments, cameras, totalStartTime, totalEndTime, addTimestamp, addMetadata, eventStartTime, progressCallback, fileHandle = null, addLogo = false, addAudio = false, colorAdjust = {}) {
        ...
        for (let i = 0; i < clipSegments.length; i++) {
            ...
            if (Object.keys(videos).length === 0) {
                throw new Error(`片段 ${i + 1} 沒有可用的影片檔案`);
            }
            ...
            if (hasVideoFrameCallback) {
                await new Promise((resolve) => {
                    ...
                    const onFrame = () => {
                        ...
                        if (processedFrames % 30 === 0) {
                            const progress = Math.min(100, Math.round((processedFrames / totalFrames) * 100));
                            progressCallback?.(`處理四宮格: ${progress}%`);
                        }
    ...
    async loadMetadataForSegments(clipSegments, camera, progressCallback = null) {
        ...
        for (let i = 0; i < clipSegments.length; i++) {
            if (this.isCancelled) throw new Error('匯出已取消');
            if (progressCallback) {
                progressCallback(`讀取影片資料 ${i + 1}/${clipSegments.length}...`);
            }
            const clipSegment = clipSegments[i];
            const videoFile = clipSegment.segment.files[camera];
...
class TeslaCamViewer {
    ...
    async handleIOSFileSelection(files) {
        ...
        if (validFiles.length === 0) {
            const lang = this.currentLanguage;
            alert(lang === 'zh-TW' 
                ? '未找到有效的 TeslaCam 影片檔案。請選擇檔案名稱格式為 "2024-01-15_12-30-00-front.mp4" 的影片檔案。'
    ...
    async loadDirectoryFromHandle(handle) {
        ...
        loadingDiv.textContent = this.currentLanguage === 'zh-TW' ? '正在載入目錄...' : 'Loading directory...';
        document.body.appendChild(loadingDiv);
    ...
    updateAllUIText(lang) {
        ...
        if (langIconEl) {
            langIconEl.textContent = lang === 'zh-TW' ? 'En' : '中';
        } else {
            this.dom.langToggleBtn.textContent = lang === 'zh-TW' ? 'En' : '中';
        }
            ...
            this.videoClipProcessor.checkFFmpeg().then(hasFFmpeg => {
                if (hasFFmpeg) {
                    ...
                    document.getElementById('useLocalFFmpegLabel').textContent = 
                        this.currentLanguage === 'zh-TW' ? '使用 FFmpeg 極速匯出' : 'Use FFmpeg Fast Export';
                } else {
                    this.dom.useLocalFFmpeg.checked = false;
                    this.dom.useLocalFFmpeg.disabled = true;
                    document.getElementById('useLocalFFmpegLabel').textContent = 
                        this.currentLanguage === 'zh-TW' ? 'FFmpeg 未安裝' : 'FFmpeg Not Installed';
                }
            });
    ...
    async saveVideoFile(blob, filename) {
        try {
            ...
            if ('showSaveFilePicker' in window) {
                ...
                this.showToast(`影片已下載: ${filename}`, 'success');
            }
        } catch (downloadError) {
            console.error('Download error:', downloadError);
            alert('下載失敗: ' + downloadError.message);
        }
    }
        ...
        try {
            ...
            const results = await this.videoClipProcessor.processClip(
                ...
                (msg) => {
                    ...
                    } else if (msg.includes('FFmpeg') || msg.includes('極速匯出') || msg.includes('寫入磁碟')) {
                    ...
                    if (result.isFile && result.path) {
                        ...
                        try {
                            ...
                            if (resolvedSavePath && resolvedSavePath !== result.path) {
                                ...
                                this.showToast(`影片已儲存到: ${result.path}`, 'success');
                            }
                        } catch (e) {
            ...
            const errorMsg = error?.message || error?.toString?.() || JSON.stringify(error) || '未知錯誤';
